...
def plot_history_from_list_split(loss1,val_loss1, loss1_label,
loss2=None,val_loss2=None,loss2_label=None,
loss3=None,val_loss3=None,loss3_label=None,loss4=None,val_loss4=None,loss4_label=None,
loss5=None,val_loss5=None,loss5_label=None,loss6=None,val_loss6=None,loss6_label=None,
loss7=None,val_loss7=None,loss7_label=None,loss8=None,val_loss8=None,loss8_label=None,
loss9=None,val_loss9=None,loss9_label=None,loss10=None,val_loss10=None,loss10_label=None,
loss11=None,val_loss11=None,loss11_label=None,loss12=None,val_loss12=None,loss12_label=None,
save=True,savefolder=None,logscale=False,ymin=None,ymax=None,title=None):

    plt.figure(figsize=(10,7))
    #Loss 1
    plt.plot(loss1,label="%s Training"%(loss1_label))
    plt.plot(val_loss1,label="%s Validation"%(loss1_label))
    #Loss 2
    plt.plot(loss2,label="%s Training"%(loss2_label))
    plt.plot(val_loss2,label="%s Validation"%(loss2_label))
    #Loss 3
    if loss3 is not None:
        plt.plot(loss3,label="%s Training"%(loss3_label))
        plt.plot(val_loss3,label="%s Validation"%(loss3_label))
    #Loss 4
    if loss4 is not None:
        plt.plot(loss4,label="%s Training"%(loss4_label))
        plt.plot(val_loss4,label="%s Validation"%(loss4_label))
        #loss 5
    if loss5 is not None:
        plt.plot(loss5,label="%s Training"%(loss5_label))
        plt.plot(val_loss5,label="%s Validation"%(loss5_label))
        #Loss 6
    if loss6 is not None:
        plt.plot(loss6,label="%s Training"%(loss6_label))
        plt.plot(val_loss6,label="%s Validation"%(loss6_label))
        #Loss 7
    if loss7 is not None:
        plt.plot(loss7,label="%s Training"%(loss7_label))
        plt.plot(val_loss7,label="%s Validation"%(loss7_label))
        #Loss 8
    if loss8 is not None:
        plt.plot(loss8,label="%s Training"%(loss8_label))
        plt.plot(val_loss8,label="%s Validation"%(loss8_label))
        #Loss 9
    if loss9 is not None:
        plt.plot(loss9,label="%s Training"%(loss9_label))
        plt.plot(val_loss9,label="%s Validation"%(loss9_label))
        #Loss 10
    if loss10 is not None:
        plt.plot(loss10,label="%s Training"%(loss10_label))
        plt.plot(val_loss10,label="%s Validation"%(loss10_label))
        #Loss 11
    if loss11 is not None:
        plt.plot(loss11,label="%s Training"%(loss11_label))
        plt.plot(val_loss11,label="%s Validation"%(loss11_label))
        #Loss 12
    if loss12 is not None:
        plt.plot(loss12,label="%s Training"%(loss12_label))
        plt.plot(val_loss12,label="%s Validation"%(loss12_label))
#Edit Axis
    if logscale:
        plt.yscale('log')
    if ymin and ymax:
    ...
    if title:
        ...
        plt.title("Training and Validation Loss after %s Epochs"%len(loss1),fontsize=25)
    plt.xlabel('Epochs',fontsize=20)
    plt.ylabel('Loss',fontsize=20)
...
def plot_distributions(truth,reco=None,label=None,save=False,savefolder=None,old_reco=None,weights=None,variable="Energy",units="(GeV)",reco_name="Retro", minval=None, maxval=None,bins=100,cnn_name="CNN",log=False,old_reco_weights=None,title=None):
    """
    ...
    plt.hist(truth, bins=bins,color='g',alpha=0.5,range=[minval,maxval],weights=weights,label=label);
    ...
    name += "T"
    if variable == 'R^2':
        plt.axvline(8100,0,1, ls ='dashed',label='R = 90 m', c = 'tab:orange')
        plt.axvline(22500,0,1,ls = 'dashed',label='R = 150 m', c = 'tab:red')
        plt.axvline(90000,0,1,ls = 'dashed',label='R = 300 m', c = 'tab:purple')
...
def plot_2D_prediction(truth, nn_reco, \
    ...
    if variable == "R Position":
      plt.vlines(x=150, ymin=0,ymax=150, linestyle='--', color="tab:red", label = "DeepCore")
      plt.vlines(x=90, ymin=0,ymax=90, linestyle='--', color="tab:green", label = "Inner DeepCore")
    if variable == "R Position":
      plt.hlines(y=150, xmin=0,xmax=150, linestyle='--', color="tab:red")
      plt.hlines(y=90, xmin=0,xmax=90, linestyle='--', color="tab:green")
      plt.legend(fontsize=20)
    ...
    if save:
        if new_labels is not None:
            plt.savefig("%s%svs%sReco%s_2DHist%s%s.png"%(savefolder,new_labels[0],new_labels[1],variable,syst_set,nocut_name),bbox_inches='tight')
        else:
...
def plot_single_resolution(truth,nn_reco,weights=None, \
    ...
    total_events = len(nn_resolution)
    ...
    if use_old_reco:
        ...
        ax.text(0.05, 0.55, textstr, transform=ax.transAxes, fontsize=20,
            verticalalignment='top', bbox=props)
...
def plot_bin_slices(truth, nn_reco, energy_truth=None, weights=None,\
    ...
    if variable == "R Position":
      plt.vlines(x=90, ymin=min(medians),ymax=max(medians), linestyle='--', color="tab:green", label = "Inner DeepCore")
      plt.vlines(x=150, ymin=min(medians),ymax=max(medians), linestyle='--', color="tab:red", label = "DeepCore")
