...
parser.add_argument("--num_files", help="Number of files to cycle through", default = 12, type = int, dest="num_files")
...
ymin = args.ymin
ymax = args.ymax
start = 49 #start epoch for plotting, because the first however many epochs are garbage
if ymin:
    ymin = float(ymin)
if ymax:
    ymax = float(ymax)
...
if len(header)-3 == 8:
    plot_history_from_list_split(data['XLoss'],data['val_XLoss'],data['YLoss'],data['val_YLoss'],data['ZLoss'],data['val_ZLoss'],save=True,savefolder=full_path,logscale=True,ymin=ymin,ymax=ymax)
if len(header)-3 > 8:
    plot_history_from_list_split(data['XLoss'],
    data['val_XLoss'],"X",data['XLoss_Error'],data['val_XLoss_Error'],"X Error",
    data['YLoss'],data['val_YLoss'],"Y",data['YLoss_Error'],data['val_YLoss_Error'], "Y Error",
    data['ZLoss'],data['val_ZLoss'], "Z",data['ZLoss_Error'],data['val_ZLoss_Error'], "Z Error",
    savefolder=full_path,logscale=True,ymin=ymin,ymax=ymax)
