...
parser.add_argument("-i", "--input_files",type=str,default=None,
                    dest="input_files", help="name of input files")
...
elif args.first_variable== "time":
    first_var = 'time'
    first_var_index = 0
    print('Training with time as the first variable.')
else:
    first_var = "energy"
    first_var_index = 0
...
use_old_reco = False
#Starting to load in the files here
files_with_paths = path + input_files
file_names = sorted(glob.glob(files_with_paths))  #glob takes in all the files found with ?? and * and sorts them, so now we have a list of the file names
...
#Just loading in one file so we get the shape of the data
afile = file_names[0]
f = h5py.File(afile, 'r')
...
x_origin = np.ones((len(X_train_DC[:,4])))*46.290000915527344
y_origin = np.ones((len(X_train_DC[:,5])))*-34.880001068115234


# LOAD MODEL from the shape
if do_error == True:
  from cnn_model_losserrorXYZ import make_network
else:
...
if first_var == "time":
    def TimeLoss(y_truth, y_predicted):
        return mean_squared_error(y_truth[:,3],y_predicted[:,0])
...
def XLoss_Error(y_truth, y_predicted):
    return mean_squared_error(tf.stop_gradient(tf.math.abs(y_truth[:,4]-y_predicted[:,0])), y_predicted[:,3])

def YLoss_Error(y_truth, y_predicted):
    return mean_squared_error(tf.stop_gradient(tf.math.abs(y_truth[:,5]-y_predicted[:,1])), y_predicted[:,4])

def ZLoss_Error(y_truth, y_predicted):
    return mean_squared_error(tf.stop_gradient(tf.math.abs(y_truth[:,6]-y_predicted[:,2])), y_predicted[:,5])

def RLoss(y_truth,y_predicted):
    r_true = np.sqrt(y_truth[:,4]**2 + y_truth[:,5]**2)
    return mean_squared_error(r_true, y_predicted[:,0])

def ZRLoss(y_truth,y_predicted): #Z when training for R and Z instead of X Y and Z
    return mean_squared_error(y_truth[:,6],y_predicted[:,1])
...
#        return energy_loss + zenith_loss + track_loss
    ...
    if first_var == "R":
        def CustomLoss(y_truth,y_predicted):
            r_loss = RLoss(y_truth,y_predicted)
            z_loss = ZRLoss(y_truth,y_predicted)
    ...
    if first_var == "time":
        def CustomLoss(y_truth,y_predicted):
            time_loss = TimeLoss(y_truth,y_predicted)
            return time_loss
...
for epoch in range(start_epoch,end_epoch):
    ...
    learning_rate = initial_lr * math.pow(lr_drop, math.floor((1+epoch)/epochs_step_drop)*no_change) #Step size for the network
    ...
    if train_variables == 1:
        ...
        if do_error == False:
          model_DC.compile(loss=CustomLoss,
              optimizer=Adam(lr=learning_rate),
              metrics=[XLoss,YLoss,ZLoss]) #Saves the individual losses so we can see if one variable is doing worse than the others
        else:
          model_DC.compile(loss=CustomLoss,
              optimizer=Adam(lr=learning_rate),
              metrics=[XLoss,YLoss,ZLoss,XLoss_Error,YLoss_Error,ZLoss_Error])
    else:
    ...
    afile.write('\n')
    afile.close() #appends to the end of the file
...
# Put all the test sets together and makes lots of plots
Y_test_use = None
X_test_DC_use = None
X_test_IC_use = None
