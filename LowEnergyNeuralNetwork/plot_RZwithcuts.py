...
parser.add_argument("--transformed", default=False,action='store_true',
                        dest='transformed',help="use flag for transformed data, to adjust max abs factor")
parser.add_argument("--cuts", default=False, action='store_true',
                    dest='cuts' help="use flag to choose whether data is cut or not"
...
minrangefrac = [-300,-300,-600]
maxrangefrac = [300,300,0]
maxrangevsPredict = [300,300,-200]
...
for i in range(3):
  ...
  NN_index= i
  recoindex= 4+i 
  maxabs_factor = mabsf #1 if not transformed, 200 if transformed
  ...
  plot_2D_prediction(Y_test_use[:,true_index]*maxabs_factor,\
                      reco_test[:,recoindex]*maxabs_factor,\
                      save,save_folder_name,bins=bins,\
